...
type ThreeClassTypes = OmitNever<{
    [K in keyof typeof THREE]: (typeof THREE)[K] extends abstract new (...args: any[]) => any ? InstanceType<(typeof THREE)[K]> extends {
        [P in `is${K}`]: boolean;
    } ? (typeof THREE)[K] : never : (typeof THREE)[K] extends {
        new (...args: any[]): any;
    } ? InstanceType<(typeof THREE)[K]> extends {
        [P in `is${K}`]: boolean;
    } ? (typeof THREE)[K] : never : never;
}>;
/**
 ...
 */
export declare const isInstanceOf: <T extends keyof ThreeClassTypes>(obj: any, type: T) => obj is InstanceType<ThreeClassTypes[T]>;
export {};
